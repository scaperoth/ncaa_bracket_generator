...
module BracketHelper
    ...
    def create_bracket(tournament, method = nil, with_comparison = false, generated = true)
        num_wrong_guesses = 0
        num_games = BracketGame.where(tournament: tournament).count
        bracket = ''
        curr_region = nil

        if generated
            generated_bracket = JSON.parse(generate_guess_bracket(tournament, method).to_json)
            num_games = generated_bracket.size
        end
        ...
        Round.where.not(number: 0).order(:number).each do |round|
            ...
            games_obj = BracketGame.where(tournament: tournament, round_id: round.id).order(:region_id, :weight)

            if games_obj.present?
                games_json = games_obj.to_json
                games = JSON.parse(games_json)
            else
                games = generated_bracket.find_all { |h1| h1['round_id'] == round.id }.sort_by { |hash| [hash['region_id'], hash['weight']] }
            end
            ...
            games.each do |game, _index|
                ...
                if generated
                    ...
                    generated_bracket.collect do |generated_bracket_game|
                        if (generated_bracket_game['region_id'] == game['region_id']) &&
                           (generated_bracket_game['round_id'] == game['round_id']) &&
    ...
    def create_team_block(team1, team2, winner, generated_winner = nil)
        ...
        team1_block = content_tag :div, class: "team #{team1_winner_accurate} " + team1_winner_class, 'data-resultid' => "team-#{team1.id}", 'data-teamid' => team1.id.to_s do
            content_tag(:label, team1.name, class: 'label') +
                content_tag(:div, team1_winner_score, class: 'score', 'data-resultid' => "result-#{team1.id}")
        end

        team2_block = content_tag :div, class: "team #{team2_winner_accurate} " + team2_winner_class, 'data-resultid' => "team-#{team2.id}", 'data-teamid' => team2.id.to_s do
